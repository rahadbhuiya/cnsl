...
| Detection metadata | MITRE ATT&CK technique tagging on built-in/correlation/Sigma rules, with a `/api/attack/coverage` aggregate report |
...
| Analysis | Graph-structured campaign detection -- transitive IP correlation through shared rules/kill-chain stages, no direct pairwise similarity needed |
...
```bash
python -m pytest tests/               # runs 1125 tests, split across tests/test_*.py by domain
```
